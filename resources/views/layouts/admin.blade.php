...
<body class="bg-slate-50 text-slate-800 font-sans antialiased min-h-screen overflow-x-hidden"
    ...
    <div class="flex min-h-screen">
        ...
        <aside
            ...
            <div class="flex h-20 items-center gap-3 border-b border-slate-800/60 px-6 shrink-0 bg-slate-900/50">
                ...
                <div class="flex flex-col text-right">
                    <span
                        class="text-sm font-black text-white tracking-tight leading-none">{{ $clinicName ?? '' }}</span>
            ...
            <nav class="flex-1 space-y-1 p-4 text-xs overflow-y-auto scrollbar-hide">
                @php
                    $menuItems = [
                        ...
                        ['route' => 'admin.users.index', 'label' => 'المدراء والمستخدمين', 'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>'],
                ...
                @foreach($menuItems as $item)
                    ...
                    <a href="{{ route($item['route']) }}"
                        class="group flex items-center gap-3.5 rounded-xl px-4 py-3 transition-all duration-300 {{ $isActive ? 'bg-white/10 text-white font-black shadow-inner border-r-2 border-primary' : 'hover:bg-white/5 hover:text-slate-200' }}">
                        <span
                            class="shrink-0 {{ $isActive ? 'text-primary' : 'text-slate-500 group-hover:text-slate-400' }} transition-colors duration-300">
                            {!! $item['icon'] !!}
                        </span>
            ...
            <form method="POST" action="{{ route('admin.logout') }}"
                class="p-4 border-t border-slate-800/60 bg-slate-900/50 shrink-0">
                @csrf
                <button type="submit"
                    class="group flex w-full items-center justify-center gap-2 rounded-xl border border-slate-800 bg-slate-800/50 px-4 py-2.5 text-xs font-black text-slate-400 hover:bg-rose-500 hover:text-white hover:border-rose-500 transition-all active:scale-97">
                    <svg class="h-4 w-4 shrink-0 text-slate-500 group-hover:text-white transition-colors" fill="none"
        ...
        <div class="flex flex-1 flex-col lg:mr-0 min-h-screen">
            <!-- Header Top bar -->
            <header
                class="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200/80 bg-white/90 backdrop-blur-xl px-6 lg:px-10 shrink-0 shadow-sm">
                <div class="flex items-center gap-4">
                    <button type="button"
                        class="lg:hidden rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-slate-650 hover:bg-slate-100 active:scale-95"
                        @click="sidebarOpen = !sidebarOpen" aria-label="القائمة">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.2"
                                d="M4 6h16M4 12h16M4 18h16" />
                        </svg>
                    </button>
                    <h1 class="text-lg font-black text-slate-900 tracking-tight">@yield('page-title', 'لوحة التحكم')
                    </h1>
                </div>
                <a href="{{ route('home') }}"
                    class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-5 py-2.5 text-xs font-black text-slate-700 shadow-sm transition-all hover:bg-white hover:border-slate-300 hover:shadow-md active:scale-97"
                    target="_blank" rel="noopener">
                    <svg class="h-3.5 w-3.5 text-primary" fill="none" stroke="currentColor" stroke-width="2.5"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div class="flex-1 p-6 lg:p-10 max-w-7xl w-full mx-auto space-y-6">
                ...
                @if (session('success'))
                    <div
                        class="flex items-center gap-3 rounded-2xl bg-emerald-50/80 p-4 border border-emerald-100 text-xs font-semibold text-emerald-800 shadow-sm">
                        <svg class="h-5 w-5 shrink-0 text-emerald-600 animate-bounce" fill="none" stroke="currentColor"
                ...
                @if (session('error'))
                    <div
                        class="flex items-center gap-3 rounded-2xl bg-rose-50/80 p-4 border border-rose-100 text-xs font-semibold text-rose-800 shadow-sm">
                        <svg class="h-5 w-5 shrink-0 text-rose-600" fill="none" stroke="currentColor" stroke-width="2.5"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div x-show="sidebarOpen" x-cloak @click="sidebarOpen = false"
        class="fixed inset-0 z-30 bg-slate-900/60 backdrop-blur-sm lg:hidden" x-transition.opacity></div>
    @stack('scripts')
</body>
