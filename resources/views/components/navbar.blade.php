<header class="sticky top-0 z-50 border-b border-slate-100 bg-white/90 backdrop-blur-md shadow-sm transition-all duration-300" x-data="{ open: false }">
    <div class="mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-4 lg:px-8">
        ...
        <nav class="hidden flex-1 items-center justify-center gap-7 md:flex">
            ...
            <a href="{{ route('offers') }}" class="relative text-[15px] font-bold transition-all hover:text-primary duration-300 py-1.5 {{ request()->routeIs('offers') ? 'text-primary after:absolute after:-bottom-[24px] after:left-0 after:right-0 after:h-[3px] after:rounded-full after:bg-primary' : 'text-slate-600' }}">العروض</a>
            <a href="{{ route('reports.login') }}" class="relative text-[15px] font-bold transition-all hover:text-primary duration-300 py-1.5 {{ request()->routeIs('reports.*') ? 'text-primary after:absolute after:-bottom-[24px] after:left-0 after:right-0 after:h-[3px] after:rounded-full after:bg-primary' : 'text-slate-600' }}">التقارير</a>
    ...
    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-4" class="absolute left-0 right-0 border-b border-slate-100 bg-white/95 backdrop-blur-md shadow-lg md:hidden">
        <nav class="flex flex-col gap-1.5 px-6 py-4 text-[15px] font-bold text-slate-700">
            ...
            <a href="{{ route('offers') }}" class="rounded-xl px-4 py-2.5 hover:bg-slate-50 hover:text-primary {{ request()->routeIs('offers') ? 'bg-primary/5 text-primary' : '' }}" @click="open = false">العروض</a>
            <a href="{{ route('reports.login') }}" class="rounded-xl px-4 py-2.5 hover:bg-slate-50 hover:text-primary {{ request()->routeIs('reports.*') ? 'bg-primary/5 text-primary' : '' }}" @click="open = false">التقارير</a>
